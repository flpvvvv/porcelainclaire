...
在运行前请导出 Supabase 凭据（与下表中的 `SUPABASE_*` 一致），可选设置 `REVALIDATION_SECRET`、`SITE_URL` 以便导入后触发站点重新验证。
...
4. 若使用 GitHub Actions 等方式跑同步，在对应环境中配置 `SUPABASE_URL`、`SUPABASE_SERVICE_KEY` 等密钥。
